...
print('Conectar ao banco do PostgreSQL')
...
print('Conectar ao banco do LTOTVS')
def conect_totvs():
    serv = '192.168.0.236'
...
WITH Qry_BenVA_Geral AS(
	SELECT 
		...
		RIGHT(dbo.SRA010.RA_DEPTO, 4) AS [Cod Local Entrega],
		'' as [Cod Centro de Custo],
		dbo.SR0010.R0_MAT AS Matricula, 
		dbo.SRA010.RA_NOMECMP AS [Nome Completo], 
  		dbo.SRA010.RA_NOME AS [Nome Impressão Cartão],
		FORMAT(CONVERT(DATE, dbo.SRA010.RA_NASC), 'dd/mm/yyyy') AS Nascimento, 
		dbo.SRA010.RA_SEXO AS Sexo, 
		'' as [Faixa Salarial],
  		dbo.SR0010.R0_VALCAL as [VR Refeição],
	...
	SELECT 
		...
		Qry_BenVA_Geral.CPF,
  		Qry_BenVA_Geral.[Cod Local Entrega],
    	Qry_BenVA_Geral.[Cod Centro de Custo],
		Qry_BenVA_Geral.Matricula,
		Qry_BenVA_Geral.[Nome Completo], 
		Qry_BenVA_Geral.[Nome Impressão Cartão],
		Qry_BenVA_Geral.Nascimento AS [DATA DE NASCIMENTO], 
		Qry_BenVA_Geral.Sexo,
  		Qry_BenVA_Geral.[Faixa Salarial],
		Qry_BenVA_Geral.[VR Refeição],
		Qry_BenVA_Geral.[VR Refeição],
  		'Final' AS Final, 
...
ticket2['Dias Calculado Corrigido'] = np.where(ticket2['Descrição do Benefício'].str.contains('DIAS'), 
                                                ticket2['Dias Prop']-ticket2['Atestados']-ticket2['Falta']-ticket2['Dias Férias'], 
                                                ticket2['Dias Prop']-ticket2['Atestados']-ticket2['Falta'])

ticket2.to_excel('F:/dp/Benefícios/2024/11_2024/VRC.xlsx')